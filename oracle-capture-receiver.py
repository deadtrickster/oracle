...
_CHAT_TASK = (
    ...
    "numbered citations.\n"
    "HOW MUCH TO LOOK AT: an open question ('tell me about this run', 'what do you think of this "
    "page') means look at all of it — a partial look produces a confident partial answer, which is "
    "worse than a slower complete one. Narrow only when the user narrows it ('briefly', 'just the "
    "errors'), and then say what you skipped. Batch the looking: tool calls you request in the SAME "
    "reply run back-to-back, so clicking through several views and screenshotting each costs one "
    "GPU swap instead of one per view.\n"
