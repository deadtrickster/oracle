...
# Also drop `Agent` (subagents): the weak model launches one async, then never awaits/collects the
# result (it barrels ahead and answers inline), so the work is wasted — and each subagent is a
# SECOND qwen fighting for the one GPU-resident model. Net negative offline. (Confirmed tool name is
# `Agent` from a real session's tool_use record.)
DISALLOW="${CLAUDE_LOCAL_DISALLOW:-mcp__codebase-memory__index_repository mcp__codebase-memory__index_status mcp__codebase-memory__detect_changes mcp__codebase-memory__ingest_traces mcp__codebase-memory__manage_adr mcp__codebase-memory__delete_project mcp__codebase-memory__get_graph_schema WebSearch WebFetch Agent}"

# shellcheck disable=SC2016  # DISCIPLINE is a literal prompt sent verbatim to the model; $-sequences must NOT be expanded
